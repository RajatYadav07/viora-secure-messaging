...
export function ChatWindow({
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 w-full bg-slate-50 dark:bg-[#0b1120] relative overflow-hidden">
      ...
      {/* Header */}
      <div className="h-16 px-4 md:px-6 border-b border-slate-200/50 dark:border-slate-800/50 bg-slate-100/80 dark:bg-slate-900/80 backdrop-blur-md flex items-center justify-between shrink-0 z-10 sticky top-0 shadow-sm">
        <div className="flex items-center space-x-3">
          {onBack && (
            <button
              onClick={onBack}
              className="md:hidden p-2 -ml-2 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200 dark:bg-slate-800 rounded-full transition-colors"
            >
          ...
          />
          <div>
            <div className="text-sm font-bold text-slate-900 dark:text-white flex items-center tracking-wide">
              {title}
            </div>
            {conversation.type === 'direct' && conversation.other_user && (
              <div className="text-[11px] text-slate-600 dark:text-slate-400 flex items-center space-x-1.5 mt-0.5">
            ...
            <div className="text-xs text-slate-600 dark:text-slate-400 flex items-center space-x-1.5 mt-0.5">
        ...
        <div className="flex items-center space-x-1.5 md:space-x-3">
          <button onClick={() => showToast('Voice calls are coming soon.', 'info')} className="p-2 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200 dark:bg-slate-800 rounded-full transition-colors">
            <Phone className="w-4 h-4" />
          </button>
          <button onClick={() => showToast('Video calls are coming soon.', 'info')} className="p-2 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200 dark:bg-slate-800 rounded-full transition-colors">
          ...
          {conversation.type === 'group' && (
            <button
              ...
              className="px-3 py-1.5 bg-slate-200/80 dark:bg-slate-800/80 hover:bg-slate-300 dark:bg-slate-700 text-slate-800 dark:text-slate-200 text-xs font-semibold rounded-full transition-all border border-slate-300/50 dark:border-slate-700/50 shadow-sm"
            >
              Group Info
            </button>
          )}
          <div onClick={() => setShowEncryptionModal(true)} className="p-2 bg-slate-200/80 dark:bg-slate-800/80 rounded-full text-slate-600 dark:text-slate-400 border border-slate-300/50 dark:border-slate-700/50 cursor-pointer hover:bg-slate-300 dark:bg-slate-700 hover:text-slate-900 dark:hover:text-white transition-all shadow-sm" title="End-to-End Encrypted">
      ...
      {/* Messages */}
      {loading ? (
        <div className="flex-1 min-h-0 flex items-center justify-center bg-slate-50 dark:bg-[#0b1120]">
          <span className="text-sm text-slate-900 dark:text-slate-500 animate-pulse">Loading messages...</span>
        </div>
      ) : (
